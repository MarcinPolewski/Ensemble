...
class Agent:
    ...
    def get_action(self, obs: dict) -> dict:
        ...
        if turn_count < 1600:
            self.guards = []
            amount = min(len(obs["allied_ships"]), 9)
            for i in range(amount):
                self.guards.append(get_id(obs["allied_ships"][i][0]))
        ...
        for ship in obs["allied_ships"]:
            ...
            if turn_count < 1600:
                ...
                if self.side == 0:
                    ship_actions.append(get_walk_action(ship_id, x, y, 90, 90, "diagonal"))
                else:
                    ship_actions.append(get_walk_action(ship_id, x, y, 10, 10, "diagonal"))
