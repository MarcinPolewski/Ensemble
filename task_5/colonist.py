...
def find_planets(id, x, y, planets, side):
    for planet in planets:
        planet_y, planet_x, planet_progress = planet  # yes, the API has the x and y mixed up, yes i had to debug this for two hours
        if abs(planet_x - x) < 9 and abs(planet_y - y) < 9:
            if (side == 0 and planet_progress != 0) or (side == 1 and planet_progress != 100):
                return get_walk_action(id, x, y, planet_x, planet_y, "x")
    return None
...
class Agent:
    ...
    def get_action(self, obs: dict) -> dict:
        ...
        for ship in obs["allied_ships"]:
            ...
            planets_found = find_planets(ship_id, x, y, obs["planets_occupation"], self.side)
            if planets_found is not None:
                ship_actions.append(planets_found)
                continue

            if strat_id > 4:
                if self.side == 0:
                    ship_actions.append(get_walk_action(ship_id, x, y, 90, 90, get_policy(strat_id)))
                else:
                    ship_actions.append(get_walk_action(ship_id, x, y, 10, 10, get_policy(strat_id)))
